...
GAME.RprView = function(engine)
{
	...
	{
		this.normalBackground = new GAME.Background(this.gameFront)//Background();
	}
	
	this.joyBackground = new GAME.JoyBackground()//Background();
	
	this.lava = new GAME.Lava(this.gameFront);

	this.powerBar = new GAME.PowerBar()//Background();
	...
	this.zoom = 1;
	//this.game.addChild(this.joyBackground);
...
GAME.RprView.prototype.showHud = function()
{
	...
	TweenLite.to(this.powerBar.position, 1, {x:GAME.width - 295, ease:Elastic.easeOut, delay:0.3})
}
...
GAME.RprView.prototype.update = function()
{
	...
	if(!GAME.lowMode)
	{
	//	this.zoom = Math.sin(this.count) + 2;
	
		//this.zoom = zoom// 1+(1-GAME.time.speed);
	...
	this.trail.target = this.engine.steve//.view.position.x;
	this.trail2.target = this.engine.steve
	//this.trail.target.y = this.engine.steve.view.position.y;
	this.trail.update();
	this.trail2.update();
	this.dust.update();
...
GAME.RprView.prototype.resize = function(w, h)
{
	...
	//this.scoreView.style.height = h/2 + "px";
	//this.scoreView.style.width = Math2.random(100, 500) + "px";
}